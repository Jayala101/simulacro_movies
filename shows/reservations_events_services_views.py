...
from .mongo_serializers import ReservationEventsServiceSerializer
...
def reservation_services_list_create(request):
    if request.method == "GET":
        q = dict(request.query_params)
        docs = [fix_id(d) for d in col.find(q)]
        return Response(docs)

    serializer = ReservationEventsServiceSerializer(data=request.data)
...
@permission_classes([IsAuthenticated])
def reservation_services_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = ReservationEventsServiceSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
